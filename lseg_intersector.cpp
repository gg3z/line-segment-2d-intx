...
compute_overlaps(vector<intvl> &intervals) {
  ...
  cout << "Sort x-interval ends = "
       << std::chrono::duration<double, std::milli>(end_sort - start_sort)
              .count()
       << endl;
#endif
  ...
  cout << "Time to construct " << ovlp_pairs.size() << " x-overlap pairs = "
       << std::chrono::duration<double, std::milli>(end_insert - start_insert)
              .count()
       << endl;
#endif
...
int LsegIntersector::numIntx(int *filtered_pairs) {
  ...
  cout << "Get x-intervals = "
       << std::chrono::duration<double, std::milli>(end_getx - start_getx)
              .count()
       << endl;
#endif

  if (dbg) {
    print_intervals(*x_intervals);
  }
  ...
  cout << "Compute Intx = "
       << std::chrono::duration<double, std::milli>(end_intx - start_intx)
              .count()
       << endl;
#endif
